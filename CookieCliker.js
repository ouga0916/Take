  ...
  let dataList = JSON.parse(localStorage.getItem("datasList")  || "[]");  // データ名のリストを保存する配列
  console.log(dataList);
  //ボタン処理
  const slBtn = document.getElementById("slBtn");

  slBtn.addEventListener("click",OpenOverlay);

  document.getElementById("olCrose").addEventListener("click",CloseOverlay);
  ...
  function OpenOverlay(){
    ...
    dataTxt.value = "データ" + (dataList.length);
    dataListDiv.innerHTML = (dataList.length > 0) ? DataListName() : "保存されたデータはありません。";
  ...
  saveBtn.addEventListener("click", () => {
    console.log(dataTxt.value);
    saveGame(dataTxt.value);
  });

  loadBtn.addEventListener("click", () => {
    console.log(dataTxt.value);
    loadGame(dataTxt.value);
  });

  deleteBtn.addEventListener("click", () => {
    deleteGame(dataTxt.value);
  });
  ...
  async function saveGame(name){
    ...
    try{
        ...
        dataList.push(name);
        localStorage.setItem("datasList", JSON.stringify(dataList));
        alert("データを保存しました!!\nデータ名:" + name);
        CloseOverlay();
    }catch(e){
      // プライベートブラウジング等で端末保存できない場合は諦める
      console.log("セーブに失敗しました: " + e);
    }
  }
